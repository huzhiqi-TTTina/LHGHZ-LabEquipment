...
/**
 * 讯飞星辰Astron AI服务实现（OpenAI兼容协议）
 * 注意：QwenLLMServiceImpl是当前的默认实现
 */
@Service
@Slf4j
public class AstronLLMServiceImpl implements LLMService {
    ...
    public AstronLLMServiceImpl(EquipmentService equipmentService,
    ...
    public String chat(String userMessage, Long userId) throws Exception {
        ...
        try (Response response = client.newCall(httpRequest).execute()) {
            if (!response.isSuccessful()) {
                ...
                log.error("AI API调用失败: HTTP {}, response: {}", response.code(), errorBody);
    ...
    private String getStatusText(String status) {
        ...
        switch (status) {
            case "NORMAL":    return "正常可用";
            case "BORROWED":  return "已被借出";
